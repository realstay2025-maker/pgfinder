...
  });
  
  const [images, setImages] = useState([]); // State holds the FileList array (actual File objects)
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');

  const handleFileChange = (e) => {
    // Store the FileList object from the input. Limit to 5 files.
...
        
        // 2. Append nested address fields flatly for easier backend parsing
...
        
        // 3. CRITICAL: Append complex roomTypes array as a JSON string
        // This ensures the array structure is preserved and can be parsed on the server.
        formData.append('roomTypes', JSON.stringify(propertyData.roomTypes)); 

        // 4. Append image files. The key 'images' MUST match the name in multer: .array('images', 5)
        images.forEach((file) => {
...
            const config = {
                headers: {
                    'Content-Type': 'multipart/form-data', // CRITICAL HEADER
...
          </section>
          
          {/* 3. Images (File Upload) */}
          <section className="border-b pb-4">
            <h2 className="text-lg md:text-xl font-semibold mb-3 text-gray-800 flex items-center">
                <PhotoIcon className="w-5 h-5 md:w-6 md:h-6 mr-2" /> Property Images (File Upload) <span className="text-custom-red">*</span>
            </h2>
...
                        {/* Temporary URL for client-side preview */}
                        <img 
...
                    </div>
                ))}
                {images.length === 0 && <p className="text-gray-500 text-xs md:text-sm">No images selected. Select property photos using the input above.</p>}
            </div>
          </section>
